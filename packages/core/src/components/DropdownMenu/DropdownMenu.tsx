...
export function DropdownMenu({
  ...
  if (menuLeft + menuWidth > win.width - gap) {
    menuLeft = Math.max(gap, win.width - menuWidth - gap);
  }
  ...
  const spaceBelow =
    win.height - anchorRect.y - anchorRect.height - gap;
  const spaceAbove = anchorRect.y - gap;
  const openBelow =
  ...
  const anchoredTop = openBelow
    ...
    : Math.max(gap, anchorRect.y - shellMaxHeight - gap);
    ...
    <View style={[styles.root, slots.root]} testID={testID}>
      <View ref={triggerRef} collapsable={false} style={{ alignSelf: 'stretch' }}>
        <Pressable
          ...
          <Text style={[styles.triggerText, slots.triggerText]}>
            {selectedLabel}
          </Text>
